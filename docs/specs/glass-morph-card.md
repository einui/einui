...
- Purpose: `GlassMorphCard` is a `Layout` component in Ein UI.
- Current maturity: `Medium`.
- Current score from COMPONENT_AUDIT.md: `49 / 100`.
...
Verified source: `registry/innovative/glass-morph-card.tsx`
...
`GlassMorphCard` is complete only if:
