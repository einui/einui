...
- Purpose: `Forgot Password Page` is a `Miscellaneous` component in Ein UI.
...
Verified source: `registry/blocks/auth/forgot-password-page.tsx`
...
```mermaid
graph TD
  A[Forgot Password Page] --> B[Layout Shell]
...
`Forgot Password Page` is complete only if:
